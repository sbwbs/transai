...
class ValkeyManager:
    ...
    def initialize_session(self, session_metadata: SessionMetadata, 
                          ttl_seconds: int = DEFAULT_SESSION_TTL) -> SessionMetadata:
        """
        Initialize session from existing SessionMetadata object
        
        Args:
            session_metadata: Pre-created SessionMetadata object
            ttl_seconds: Session TTL in seconds
            
        Returns:
            SessionMetadata object
        """
        with self._operation_timer(f"initialize_session_{session_metadata.doc_id}"):
            doc_id = session_metadata.doc_id
            
            # Store session metadata with TTL
            session_key = f"{self.DOC_META_PREFIX}:{doc_id}:metadata"
            session_data = self._serialize_data(session_metadata)
            ...
            self.logger.info(f"Initialized session for document {doc_id} with {session_metadata.total_segments} segments")
